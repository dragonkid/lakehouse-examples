...
class DataStatisticsOperator<D extends DataStatistics<D, S>, S>
        extends AbstractStreamOperator<DataStatisticsOrRecord<D, S>> implements
        OneInputStreamOperator<RowData, DataStatisticsOrRecord<D, S>>, OperatorEventHandler {
    ...
    DataStatisticsOperator(KeySelector<RowData, RowData> keySelector,
    ...
    public void initializeState(StateInitializationContext context) throws Exception {
        ...
        globalStatisticsState = context.getOperatorStateStore().getUnionListState(
    ...
    public void open() throws Exception {
        if (!globalStatistics.isEmpty()) {
            output.collect(new StreamRecord<>(
                    DataStatisticsOrRecord.fromDataStatistics(globalStatistics)));
        }
    }
    ...
    public void snapshotState(StateSnapshotContext context) throws Exception {
        ...
        LOG.info("Taking data statistics operator snapshot for checkpoint {} in subtask {}",
        ...
        if (!globalStatistics.isEmpty() && getRuntimeContext().getIndexOfThisSubtask() == 0) {
            ...
            LOG.info("Saving global statistics {} to state in subtask {}", globalStatistics,
                    subTaskId);
        ...
        operatorEventGateway
                .sendEventToCoordinator(new DataStatisticsEvent<>(checkpointId, localStatistics));
