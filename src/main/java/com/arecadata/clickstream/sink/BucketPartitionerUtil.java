...
final class BucketPartitionerUtil {
    ...
    private static Tuple2<Integer, Integer> getBucketFieldInfo(PartitionSpec partitionSpec) {
        List<Tuple2<Integer, Integer>> bucketFields = getBucketFields(partitionSpec);
        Preconditions.checkArgument(bucketFields.size() == 1,
                BucketPartitionerUtil.BAD_NUMBER_OF_BUCKETS_ERROR_MESSAGE, bucketFields.size());
    ...
    private static class BucketPartitionSpecVisitor
        ...
        @Override
        public Tuple2<Integer, Integer> bucket(int fieldId, String sourceName, int sourceId,
                int numBuckets) {
            return new Tuple2<>(fieldId, numBuckets);
        }

        @Override
        public Tuple2<Integer, Integer> truncate(int fieldId, String sourceName, int sourceId,
                int width) {
            return null;
        }
        ...
        @Override
        public Tuple2<Integer, Integer> unknown(int fieldId, String sourceName, int sourceId,
                String transform) {
            return null;
        }
    }
