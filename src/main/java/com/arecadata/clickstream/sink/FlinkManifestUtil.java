...
class FlinkManifestUtil {
    ...
    static ManifestFile writeDataFiles(OutputFile outputFile, PartitionSpec spec,
    ...
    static List<DataFile> readDataFiles(ManifestFile manifestFile, FileIO io,
            Map<Integer, PartitionSpec> specsById) throws IOException {
    ...
    static ManifestOutputFileFactory createOutputFileFactory(Table table, String flinkJobId,
            String operatorUniqueId, int subTaskId, long attemptNumber) {
        TableOperations ops = ((HasTableOperations) table).operations();
        return new ManifestOutputFileFactory(ops, table.io(), table.properties(), flinkJobId,
                operatorUniqueId, subTaskId, attemptNumber);
    }
    ...
    static DeltaManifests writeCompletedFiles(WriteResult result,
            Supplier<OutputFile> outputFileSupplier, PartitionSpec spec) throws IOException {
        ...
        if (result.deleteFiles() != null && result.deleteFiles().length > 0) {
            ...
            ManifestWriter<DeleteFile> deleteManifestWriter = ManifestFiles
                    .writeDeleteManifest(FORMAT_V2, spec, deleteManifestFile, DUMMY_SNAPSHOT_ID);
    ...
    static WriteResult readCompletedFiles(DeltaManifests deltaManifests, FileIO io,
        ...
        if (deltaManifests.deleteManifest() != null) {
            try (CloseableIterable<DeleteFile> deleteFiles = ManifestFiles
                    .readDeleteManifest(deltaManifests.deleteManifest(), io, specsById)) {
