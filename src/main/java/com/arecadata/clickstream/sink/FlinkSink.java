 ...
 * the License.
 */
package com.arecadata.clickstream.sink;
...
public class FlinkSink {
     ...
     */
    public static <T> Builder builderFor(DataStream<T> input, MapFunction<T, RowData> mapper,
     ...
     * {@link TableSchema} for builder to convert those {@link Row}s to a {@link RowData}
     * DataStream.
     *
    ...
    public static Builder forRow(DataStream<Row> input, TableSchema tableSchema) {
        ...
        return builderFor(input, rowConverter::toInternal,
                FlinkCompatibilityUtil.toTypeInfo(rowType)).tableSchema(tableSchema);
    }
    ...
    public static class Builder {
        ...
        private <T> Builder forMapperOutputType(DataStream<T> input, MapFunction<T, RowData> mapper,
                TypeInformation<RowData> outputType) {
        ...
        /**
         * Set the write properties for Flink sink. View the supported properties in
         * {@link FlinkWriteOptions}
         */
        ...
        public Builder writeParallelism(int newWriteParallelism) {
            writeOptions.put(FlinkWriteOptions.WRITE_PARALLELISM.key(),
         ...
         * <br>
         * Flink auto generates operator uid if not set explicitly. It is a recommended <a href=
         * "https://ci.apache.org/projects/flink/flink-docs-master/docs/ops/production_ready/">
         ...
         * specifically the committer operator state). You need to use
         * {@code --allowNonRestoredState} to ignore the previous sink state. During restore Flink
         * sink state is used to check if last commit was actually successful or not.
         * {@code --allowNonRestoredState} can lead to data loss if the Iceberg commit failed in the
        ...
        private <T> DataStreamSink<T> chainIcebergOperators() {
            ...
            // and
            // equality fields.
            DataStream<RowData> distributeStream = distributeDataStream(rowDataInput,
                    equalityFieldIds, table.spec(), table.schema(), flinkRowType);
        ...
        private <T> DataStreamSink<T> appendDummySink(
            ...
            DataStreamSink<T> resultStream = committerStream.addSink(new DiscardingSink())
        ...
        private SingleOutputStreamOperator<Void> appendCommitter(
                SingleOutputStreamOperator<WriteResult> writerStream) {
            IcebergFilesCommitter filesCommitter = new IcebergFilesCommitter(tableLoader,
                    flinkWriteConf.overwriteMode(), snapshotProperties,
                    flinkWriteConf.workerPoolSize(), flinkWriteConf.branch(), table.spec());
            SingleOutputStreamOperator<Void> committerStream =
                    writerStream.transform(operatorName(ICEBERG_FILES_COMMITTER_NAME), Types.VOID,
                            filesCommitter).setParallelism(1).setMaxParallelism(1);
            if (uidPrefix != null) {
        ...
        private SingleOutputStreamOperator<WriteResult> appendWriter(DataStream<RowData> input,
                RowType flinkRowType, List<Integer> equalityFieldIds) {
            ...
            int parallelism = flinkWriteConf.writeParallelism() == null ? input.getParallelism()
        ...
        private DataStream<RowData> distributeDataStream(DataStream<RowData> input,
                List<Integer> equalityFieldIds, PartitionSpec partitionSpec, Schema iSchema,
                    ...
                    if (equalityFieldIds.isEmpty()) {
                        if (partitionSpec.isUnpartitioned()) {
                            ...
                            if (BucketPartitionerUtil.hasOneBucketField(partitionSpec)) {
                                return input.partitionCustom(new BucketPartitioner(partitionSpec),
    ...
    static RowType toFlinkRowType(Schema schema, TableSchema requestedSchema) {
        if (requestedSchema != null) {
            ...
            // SMALLINT will
            // be promoted to
            // iceberg INTEGER, that means if we use iceberg's table schema to read TINYINT (backend
            // by 1
            // 'byte'), we will
            // read 4 bytes rather than 1 byte, it will mess up the byte array in BinaryRowData. So
            // here
            // we must use flink
            // schema.
    ...
    static IcebergStreamWriter<RowData> createStreamWriter(Table table,
            FlinkWriteConf flinkWriteConf, RowType flinkRowType, List<Integer> equalityFieldIds) {
        ...
        TaskWriterFactory<RowData> taskWriterFactory = new RowDataTaskWriterFactory(
                serializableTable, flinkRowType, flinkWriteConf.targetDataFileSize(), format,
                writeProperties(table, format, flinkWriteConf), equalityFieldIds,
     ...
     */
    private static Map<String, String> writeProperties(Table table, FileFormat format,
            FlinkWriteConf conf) {
