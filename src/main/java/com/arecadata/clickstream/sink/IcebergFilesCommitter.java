...
class IcebergFilesCommitter extends AbstractStreamOperator<Void>
    ...
    IcebergFilesCommitter(TableLoader tableLoader, boolean replacePartitions,
            Map<String, String> snapshotProperties, Integer workerPoolSize, String branch,
            PartitionSpec spec) {
    ...
    public void initializeState(StateInitializationContext context) throws Exception {
        ...
        this.manifestOutputFileFactory = FlinkManifestUtil.createOutputFileFactory(table,
        ...
        if (context.isRestored()) {
            ...
            // the new
            // flink job even if it's restored from a snapshot created by another different flink
            // job, so
    ...
    public void notifyCheckpointComplete(long checkpointId) throws Exception {
        ...
        if (checkpointId > maxCommittedCheckpointId) {
            ...
            LOG.info("Skipping committing checkpoint {}. {} is already committed.", checkpointId,
    ...
    private void commitUpToCheckpoint(NavigableMap<Long, byte[]> deltaManifestsMap,
        ...
        for (Map.Entry<Long, byte[]> e : pendingMap.entrySet()) {
            ...
            pendingResults.put(e.getKey(), FlinkManifestUtil.readCompletedFiles(deltaManifests,
    ...
    private void commitPendingResult(NavigableMap<Long, WriteResult> pendingResults,
            CommitSummary summary, String newFlinkJobId, String operatorId, long checkpointId) {
        ...
        if (totalFiles != 0 || continuousEmptyCheckpoints % maxContinuousEmptyCommits == 0) {
            ...
            LOG.info("Skip commit for checkpoint {} due to no data files or delete files.",
                    checkpointId);
        }
    }

    private void deleteCommittedManifests(List<ManifestFile> manifests, String newFlinkJobId,
            long checkpointId) {
        for (ManifestFile manifest : manifests) {
            try {
                ...
                String details = MoreObjects.toStringHelper(this).add("flinkJobId", newFlinkJobId)
                        .add("checkpointId", checkpointId).add("manifestPath", manifest.path())
                        .toString();
                LOG.warn(
    ...
    private void replacePartitions(NavigableMap<Long, WriteResult> pendingResults,
            CommitSummary summary, String newFlinkJobId, String operatorId, long checkpointId) {
        Preconditions.checkState(summary.deleteFilesCount() == 0,
        ...
        commitOperation(dynamicOverwrite, summary, "dynamic partition overwrite", newFlinkJobId,
    ...
    private void commitDeltaTxn(NavigableMap<Long, WriteResult> pendingResults,
        ...
        if (summary.deleteFilesCount() == 0) {
            ...
            commitOperation(appendFiles, summary, "append", newFlinkJobId, operatorId,
                    checkpointId);
        } else {
            ...
            for (Map.Entry<Long, WriteResult> e : pendingResults.entrySet()) {
                ...
                commitOperation(rowDelta, summary, "rowDelta", newFlinkJobId, operatorId,
                        e.getKey());
            }
        }
    }

    private void commitOperation(SnapshotUpdate<?> operation, CommitSummary summary,
        ...
        LOG.info("Committing {} for checkpoint {} to table {} branch {} with summary: {}",
                description, checkpointId, table.name(), branch, summary);
        ...
        LOG.info("Committed {} to table: {}, branch: {}, checkpointId {} in {} ms", description,
                table.name(), branch, checkpointId, durationMs);
    ...
    private byte[] writeToManifest(long checkpointId) throws IOException {
        ...
        DeltaManifests deltaManifests = FlinkManifestUtil.writeCompletedFiles(result,
                () -> manifestOutputFileFactory.create(checkpointId), spec);

        return SimpleVersionedSerialization
                .writeVersionAndSerialize(DeltaManifestsSerializer.INSTANCE, deltaManifests);
    }
    ...
    static ListStateDescriptor<SortedMap<Long, byte[]>> buildStateDescriptor() {
        ...
        SortedMapTypeInfo<Long, byte[]> sortedMapTypeInfo =
                new SortedMapTypeInfo<>(BasicTypeInfo.LONG_TYPE_INFO,
                        PrimitiveArrayTypeInfo.BYTE_PRIMITIVE_ARRAY_TYPE_INFO, longComparator);
