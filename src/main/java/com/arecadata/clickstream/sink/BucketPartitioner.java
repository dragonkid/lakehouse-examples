...
class BucketPartitioner implements Partitioner<Integer> {
     ...
     * (one writer -> many buckets). Conversely, if the number of writers > the number of buckets
     * the logic is handled by the {@link #getPartitionWithMoreWritersThanBuckets
    ...
    public int partition(Integer bucketId, int numPartitions) {
        ...
        Preconditions.checkArgument(bucketId < maxNumBuckets,
                BUCKET_GREATER_THAN_UPPER_BOUND_MESSAGE, bucketId, maxNumBuckets);
